...
Y_train_scaled_C = Y_train_scaled[:, 0]
Y_train_scaled_S = Y_train_scaled[:, 1]

Y_val_scaled_C = Y_val_scaled[:, 0]
Y_val_scaled_S = Y_val_scaled[:, 1]
...
history = model.fit(
    X_train_scaled, 
    Y_train_scaled_S,
    validation_data=(X_val_scaled, Y_val_scaled_S),
    epochs=epochs,
    batch_size=batch_size,
    callbacks=[early_stop, reduce_lr]
)

val_loss, val_mae = model.evaluate(X_val_scaled, Y_val_scaled_S, verbose=0)
...
predictions_original[:, 0] = scaler_Y_1.inverse_transform(predictions[:, 0].reshape(-1, 1)).flatten()

true_S = Y_test[:, 1]
pred_S = predictions_original[:, 0]
...
result_df = pd.DataFrame([[mean_S, std_S]], columns=["mean_S", "std_S"])
result_df.to_csv("32_16_8_1_model_200_s_1ns.csv", mode='a', header=False, index=False)

print(f"Experiment completed: mean_S={mean_S:.4f}, std_S={std_S:.4f}")
