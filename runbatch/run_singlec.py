...
history = model.fit(
    X_train_scaled, 
    Y_train_scaled_C,
    validation_data=(X_val_scaled, Y_val_scaled_C),
    epochs=epochs,
    batch_size=batch_size,
    callbacks=[early_stop, reduce_lr]
)

val_loss, val_mae = model.evaluate(X_val_scaled, Y_val_scaled_C, verbose=0)
...
result_df = pd.DataFrame([[mean_C, std_C]], columns=["mean_C", "std_C"])
result_df.to_csv("32_16_8_1_model_100_c_1ns.csv", mode='a', header=False, index=False)

print(f"Experiment completed: mean_C={mean_C:.4f}, std_C={std_C:.4f}")
